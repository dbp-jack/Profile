      ...
      <div className="relative mx-auto max-w-screen-2xl px-10 lg:px-16">
        ...
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 xl:grid-cols-3">
          {ABOUT_CARDS.map((card) => (
            <article
              key={card.title}
              className={`group relative flex h-full flex-col overflow-hidden rounded-2xl border px-8 py-8 sm:px-9 sm:py-9 md:px-10 md:py-10 shadow-sm transition-all duration-300 ${
                dark
            ...
            >
              ...
              <div className="relative flex min-h-[6rem] items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className={`text-2xl font-extrabold tracking-tight ${dark ? 'text-[#e8e8e8]' : 'text-gray-900'}`}>
                    {card.title}
                  </h3>
                  {'subtitle' in card && card.subtitle ? (
                    <p
                      className={`mt-1 text-sm font-semibold uppercase tracking-[0.14em] ${dark ? 'text-[#7a9fd4]' : 'text-[#2563EB]'}`}
                    >
                      {card.subtitle}
                    </p>
                  ) : null}
              ...
              <div
                className={`relative mt-1 space-y-4 text-sm leading-relaxed ${dark ? 'text-[#b8b8b8]' : 'text-gray-600'}`}
              >
