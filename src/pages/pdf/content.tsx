...
export const draftPages: DraftPage[] = [
  {
    ...
    body: <div className="rac-cover"><img className="rac-photo" src={asset('profile-photo.png')} alt="정민수 증명사진" /><div><p className="cover-role">백엔드 개발자</p><h2>정민수</h2><p className="cover-intro">수치로 검증하고, 팀 흐름을 움직이는 백엔드 개발자</p><dl className="rac-contact">{HERO_PERSONAL_INFO.map((row, i) => <div key={row.text}><dt>{['주소', '전화번호', 'Github', 'LinkedIn'][i]}</dt><dd>{row.href ? <a href={row.href} target="_blank" rel="noreferrer">{row.text}</a> : row.text}</dd></div>)}</dl><div className="rac-skills">{HERO_SKILL_GROUPS.map(group => <div key={group.label}><strong>{group.label}</strong><span>{group.tags.join(' · ')}</span></div>)}<div><strong>AI</strong><span>Local Agent · Ollama · ChromaDB · Codex</span></div></div></div></div>,
  },
  {
