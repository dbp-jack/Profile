...
export const fixTicketingProject: ProjectData = {
  ...
  overviewSummary: {
    intro: 'MSA 야구 티켓팅에서 주문 도메인, 결제 모듈, Kafka 이벤트 흐름을 맡았습니다.',
    achievement: '주문·결제 동기 호출의 장애 전파 문제를 Kafka 이벤트로 분리해, 실패·취소 보상 흐름까지 연결',
    reflection: '이벤트 설계는 실패 이후 책임과 상태 전이를 먼저 정해야 합니다.',
  },
  serviceOverview:
  ...
  projectReflection: {
    title: '실패 이후 상태까지 포함하는 이벤트 설계',
    body:
      '티켓팅에서 주문과 결제가 동기 호출로 묶이면 결제 지연이나 실패가 예약 흐름까지 전파되어, 사용자가 주문 상태를 신뢰하기 어려워질 수 있었습니다.<br/>이에 주문과 결제를 Kafka 이벤트로 분리하고 성공·실패·취소 이벤트를 나누어, 결제 처리를 기다리지 않으면서도 실패 시 주문 취소로 이어지는 보상 흐름을 구현했습니다.<br/>이 경험을 통해 이벤트 기반 설계는 단순히 비동기로 전환하는 작업이 아니라, 실패했을 때 어느 서비스가 어떤 상태를 책임지는지 먼저 정의하는 일이라는 기준을 갖게 되었습니다.',
  },
