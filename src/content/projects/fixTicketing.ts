...
export const fixTicketingProject: ProjectData = {
  ...
  projectReflection: {
    title: '이벤트는 실패 이후의 상태까지 이어져야 했습니다',
    body:
      '티켓팅에서 주문과 결제가 동기 호출로 묶이면 결제 지연이나 실패가 예약 흐름까지 끌고 들어와, 사용자는 주문 상태를 신뢰하기 어려워질 수 있다고 봤습니다.<br/>주문·결제를 Kafka 이벤트로 분리하고 성공·실패·취소 이벤트를 나눠, 결제 처리 지연을 기다리지 않으면서도 실패 시 주문 취소로 이어지는 보상 흐름을 만들었습니다.<br/>이 경험으로 이벤트 기반 설계는 비동기로 바꾸는 기술 선택보다, 실패했을 때 누가 어떤 상태를 책임지는지 먼저 정하는 일이라고 느꼈습니다.',
  },
