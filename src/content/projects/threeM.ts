...
export const threeMProject: ProjectData = {
    ...
    thinking:
      '인증·사용자 <span class="font-semibold">모듈 경계</span>와 요청마다 발생하는 <span class="font-semibold">User 재호출</span>을 함께 줄이기 위해 대안을 비교했습니다.<div class="mt-3 overflow-x-auto rounded-xl border border-slate-200 dark:border-[#3f3f45]"><table class="w-full min-w-[760px] table-fixed border-collapse text-left text-sm"><thead class="bg-slate-100 text-slate-600 dark:bg-[#33333a] dark:text-slate-300"><tr><th class="w-[24%] px-3 py-2 font-bold">검토안</th><th class="w-[24%] px-3 py-2 font-bold">장점</th><th class="w-[37%] px-3 py-2 font-bold">한계</th><th class="w-[15%] px-3 py-2 font-bold">판단</th></tr></thead><tbody class="bg-white text-slate-700 dark:bg-[#2f2f34] dark:text-slate-200"><tr class="border-t border-slate-200 dark:border-[#45454d]"><td class="px-3 py-2 font-semibold">Auth·User 책임 혼재</td><td class="px-3 py-2">구현·공유가 간단함</td><td class="px-3 py-2">인증 정책 변경이 User 배포까지 확산</td><td class="px-3 py-2 font-bold text-red-600 dark:text-red-400">제외</td></tr><tr class="border-t border-slate-200 dark:border-[#45454d]"><td class="px-3 py-2 font-semibold">매 요청 User 재조회</td><td class="px-3 py-2">최신 role 즉시 반영</td><td class="px-3 py-2">네트워크 호출 증가·User 장애가 인증 전체로 전파</td><td class="px-3 py-2 font-bold text-red-600 dark:text-red-400">제외</td></tr><tr class="border-t border-slate-200 dark:border-[#45454d]"><td class="px-3 py-2 font-semibold">Gateway 로컬 캐시</td><td class="px-3 py-2">User 호출 감소</td><td class="px-3 py-2">스케일아웃 시 캐시 불일치·무효화 관리</td><td class="px-3 py-2 font-bold text-red-600 dark:text-red-400">제외</td></tr><tr class="border-t border-blue-200 bg-blue-50/70 dark:border-blue-900/50 dark:bg-blue-950/20"><td class="px-3 py-2 font-bold text-[#2563EB] dark:text-[#8aa8e8]">Auth/User 분리 + JWT 컨텍스트</td><td class="px-3 py-2">변경 경계 분리·재조회 없이 Gateway 권한 판단</td><td class="px-3 py-2">토큰 만료 전 role 변경 지연</td><td class="px-3 py-2 font-bold text-[#2563EB] dark:text-[#8aa8e8]">최종 선택</td></tr></tbody></table></div><div class="mt-3 rounded-lg border border-amber-200 bg-amber-50/80 px-4 py-3 font-semibold leading-relaxed text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-200">인증과 사용자의 변경 경계를 분리하고, JWT 컨텍스트로 User 재조회를 제거해 장애 전파 범위를 줄였습니다.</div>',
    solution:
          '<div class="space-y-3"><div class="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-[#3f3f45] dark:bg-[#2f2f34]"><div class="text-lg font-extrabold text-slate-900 dark:text-slate-100">1단계 — 서비스 경계 분리</div><div class="mt-2 space-y-1 text-base leading-relaxed text-slate-800 dark:text-slate-200"><div>인증은 <span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">Auth</span> 모듈, 사용자 관리는 <span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">User</span> 모듈로 분리 → 서비스 경계 명확화</div><div><span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">Auth</span> 로그인/회원가입 처리 후, JWT(access/refresh) 발급, 토큰에 <span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">userId · role</span> 포함</div></div><div class="mt-4"><img loading="lazy" src="3m-auth-user-class-diagram.png" alt="Auth/User 도메인 분리 클래스 구조" class="w-full rounded-lg border border-slate-200 object-contain dark:border-[#4a4a52]"/><p class="mt-1.5 text-right text-xs text-slate-500 dark:text-slate-400">Auth/User 도메인 분리 클래스 구조 — Auth는 인증에 필요한 최소 정보만 보관, Feign으로 단방향 통신</p></div></div><div class="rounded-xl border border-amber-200 bg-amber-50/80 px-4 py-3 dark:border-amber-900/40 dark:bg-amber-950/20"><div class="mb-2 text-sm font-bold text-amber-700 dark:text-amber-300">JWT payload 설계 대안 비교</div><div class="space-y-2 text-sm leading-relaxed text-slate-700 dark:text-slate-300"><div><span class="font-semibold text-red-600 dark:text-red-400">대안 1: 매 요청마다 Auth 서비스 재조회</span><br/>→ Auth 서비스가 SPOF(단일 장애점), 네트워크 왕복 지속 발생</div><div><span class="font-semibold text-red-600 dark:text-red-400">대안 2: Auth 서비스 내 로컬 캐시</span><br/>→ 스케일아웃 시 인스턴스 간 캐시 불일치, 무효화 시점 관리 복잡</div><div><span class="font-semibold text-[#2563EB] dark:text-[#8aa8e8]">선택: JWT payload에 userId·role 포함</span><br/>→ 각 서비스가 토큰 검증만으로 인가 처리 완결, stateless 구조 유지</div><div class="mt-1 rounded-md border border-slate-200 bg-white/70 px-2.5 py-2 text-xs text-slate-500 dark:border-[#3f3f45] dark:bg-[#2f2f34] dark:text-slate-400"><span class="font-semibold">한계:</span> blacklist 미구현으로 로그아웃 후 access token 만료 전(1시간)까지 유효. role 변경 시 기존 토큰 즉시 무효화 불가. 즉시 무효화가 필요한 경우 Redis blacklist 전략이 필요.</div></div></div><div class="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-[#3f3f45] dark:bg-[#2f2f34]"><div class="text-lg font-extrabold text-slate-900 dark:text-slate-100">2단계 — 인증 흐름 단순화</div><div class="mt-2 space-y-1 text-base leading-relaxed text-slate-800 dark:text-slate-200"><div>Gateway 인증 필터에서 JWT 검증 후 <span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">X-User-*</span> 헤더로 사용자 컨텍스트 전달</div><div><span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">User</span> 서비스 재호출 없이 Gateway에서 권한 판단, 추가 정보 필요 시에만 선택 호출</div><div>권한 검증은 AOP(<span class="rounded-md bg-slate-200/80 px-2 py-0.5 font-mono text-sm dark:bg-[#3a3a45]">@RequiresMasterRole</span>)로 분리 → 역할 체크 로직 통합</div></div></div></div>',
    result:
    ...
    projectReflection: {
      ...
      body:
        '허브 관리자 권한을 Gateway에서 일관되게 판단하려 했지만, 초기에는 제외 경로가 <span class="font-mono font-semibold">/api/v1/users</span> 전체를 건너뛰고 User 서비스에 <span class="font-mono font-semibold">MasterRoleAspect</span>도 없어 권한 검증이 우회될 수 있었습니다.<br/>통합 테스트에서 이를 확인해 제외 경로를 축소하고, <span class="font-mono font-semibold">X-User-Role</span> 전달과 AOP·예외 처리를 추가해 <strong>MASTER 200·HUB_MANAGER 403·미인증 401</strong>을 검증했습니다.<br/>그 결과 권한 설계는 JWT 발급만으로 끝나지 않고, Gateway 필터부터 서비스 권한 판단과 HTTP 응답까지 전체 경로로 검증하게 됐습니다.',
      sourceUrl:
