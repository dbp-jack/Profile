...
export const threeMProject: ProjectData = {
    ...
    result:
          '<ul class="space-y-2 list-none pl-0"><li class="flex gap-2"><span class="mt-1 text-slate-400">•</span><span>UserService → Auth 도메인 <span class="font-semibold text-emerald-600 dark:text-emerald-400">결합도(CBO) 0건</span> — 인증 정책 변경 시 User 모듈 배포 영향 제거</span></li><li class="flex gap-2"><span class="mt-1 text-slate-400">•</span><span>Auth → User 의존은 Feign 호출용 DTO 중심의 단방향 참조로 제한 — <span class="font-semibold text-emerald-600 dark:text-emerald-400">순환 의존 없음</span></span></li><li class="flex gap-2"><span class="mt-1 text-slate-400">•</span><span>인증 정책·사용자 정책 변경 범위를 각 모듈로 수렴, 한 도메인 수정이 다른 도메인 배포로 이어지지 않는 구조 확보</span></li></ul><div class="mt-3 rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2 text-sm leading-relaxed text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-200"><strong>현재 한계:</strong> 토큰 만료 전 role 변경·탈퇴 즉시 반영과 blacklist는 구현되어 있지 않음.</div><p class="mt-3 text-xs text-slate-500 dark:text-slate-400">※ CBO: 소스 코드 import 정적 분석으로 측정 — 실제 코드에서 참조된 외부 클래스 수 기준</p>',
    projectReflection: {
