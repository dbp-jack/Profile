...
export const PROJECTS: ProjectData[] = [
  {
    ...
    problemSections: [
      {
        headline: '섹션 1 — 이벤트 목록 조회 성능 개선',
        problem:
          '이벤트 목록 조회 시 연관 데이터(eventDetail, rewards) 로딩으로 <span class="font-semibold text-red-600 dark:text-red-300">N+1 쿼리 발생</span><br/>Vuser 100 기준 평균 응답시간 <span class="font-semibold text-red-600 dark:text-red-300">645ms</span>, TPS <span class="font-semibold text-red-600 dark:text-red-300">154.6</span> 한계<br/>Vuser 1,000 기준 평균 응답시간 <span class="font-semibold text-red-600 dark:text-red-300">6,818ms</span>, TPS <span class="font-semibold text-red-600 dark:text-red-300">138.7</span> 한계<br/>findAll() 기반 전체 조회 후 메모리 필터링 구조 → 데이터 증가 시 성능 저하 우려<div class="mt-3"><img src="before-scouter-sql42.png" alt="Scouter XLog SQL Count 42" class="w-full rounded-lg border border-slate-200 object-contain dark:border-[#4a4a52]"/></div>',
        thinking:
          '이벤트 투표는 단기간에 동시 요청이 집중되고, 특히 랭킹 상위 게시물일수록 쏠림이 심해집니다.<br /> 또한, 목록 조회(반복 읽기)와 투표 수(빈번한 갱신·동시성)는 부하 축이 달라, 쿼리 최적화와 캐시·원자 연산을 같은 문제로 보지 않았습니다.<br /> 문제를 두 레이어로 분리해 접근했습니다.<div class="mt-3 rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-[#3f3f45] dark:bg-[#2f2f34]"><div class="font-semibold text-slate-800 dark:text-slate-100">1. 쿼리 자체의 비효율 → 근본적인 쿼리 개선 필요</div><div class="mt-2 font-semibold text-slate-800 dark:text-slate-100">2. 반복 조회 비용 → 데이터 특성별 캐시 전략 분리 필요</div></div><div class="mt-3 flex items-start gap-3"><span class="rounded-md bg-blue-100 px-2.5 py-1 text-sm font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">이벤트 목록</span><span class="min-w-0 pt-0.5">조회 많음 / 변경 적음 → 캐싱 적합<div class="mt-1 text-sm text-slate-500 dark:text-slate-400">(사용자 탐색 단계에서 반복 노출되는 데이터 — 변경 빈도가 낮아 캐시 유효)</div></span></div><div class="mt-2 flex items-start gap-3"><span class="rounded-md bg-blue-100 px-2.5 py-1 text-sm font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">투표 수</span><span class="min-w-0 pt-0.5">증감 빈번 / 동시성 중요 → 원자적 연산 필요<div class="mt-1 text-sm text-slate-500 dark:text-slate-400">(랭킹 경쟁 중 상위 게시물에 투표가 몰리는 패턴 → 정합성 보장이 핵심)</div></span></div><div class="mt-3 font-semibold text-amber-600 dark:text-amber-300">👉 단일 캐시 전략이 아니라 역할별 캐시 분리가 필요하다고 판단</div>',
        solution:
          '<div class="space-y-3"><div class="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-[#3f3f45] dark:bg-[#2f2f34]"><div class="text-lg font-extrabold text-slate-900 dark:text-slate-100">1단계 — 쿼리 최적화</div><div class="mt-2 space-y-1 text-base leading-relaxed text-slate-800 dark:text-slate-200"><div>QueryDSL 기반 <span class="font-semibold">EventQueryRepositoryImpl</span> 도입</div><div>eventDetail, rewards를 <span class="font-mono">leftJoin + fetchJoin</span>으로 한 번에 조회해 N+1 제거</div><div>페이징 시 조인 중복 문제를 <span class="font-mono">countDistinct</span>로 count 쿼리 분리해 보정</div></div></div><div class="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-[#3f3f45] dark:bg-[#2f2f34]"><div class="text-lg font-extrabold text-slate-900 dark:text-slate-100">2단계 — 역할별 캐시 전략 분리</div><div class="mt-2 space-y-1.5 text-base leading-relaxed text-slate-800 dark:text-slate-200"><div><span class="mr-2 rounded-md bg-blue-100 px-2.5 py-1 text-sm font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">이벤트 목록</span>@Cacheable 적용, ConcurrentMapCacheManager → RedisCacheManager 전환으로 분산 환경 대응</div><div><span class="mr-2 rounded-md bg-blue-100 px-2.5 py-1 text-sm font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">투표 수</span>Redis INCR/DECR 원자적 연산으로 동시성 문제 없이 정확성 보장</div><div>TTL 및 @CacheEvict 적용 / Cache Miss 시 DB → 캐시 재적재 구조 유지</div></div></div><div class="mt-3 overflow-hidden rounded-xl border border-slate-200/95 bg-white dark:border-[#42424c] dark:bg-[#2c2c34]"><table class="w-full border-collapse text-left text-base text-slate-700 dark:text-slate-300"><caption class="sr-only">Scouter SQL Count 비교</caption><thead><tr class="border-b border-slate-200 bg-slate-50 dark:border-[#40404a] dark:bg-[#32323a]"><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">단계</th><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">SQL Count</th><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">Scouter 화면</th></tr></thead><tbody><tr><td class="border-t border-slate-100 px-3 py-2 text-slate-700 dark:border-[#3a3a44] dark:text-slate-300">Before (N+1)</td><td class="border-t border-slate-100 px-3 py-2 font-semibold text-red-600 dark:border-[#3a3a44] dark:text-red-400">42번</td><td class="border-t border-slate-100 px-3 py-2 dark:border-[#3a3a44]"><img src="before-scouter-sql42.png" alt="Before SQL 42" class="h-auto max-h-24 rounded object-contain"/></td></tr><tr><td class="border-t border-slate-100 px-3 py-2 text-slate-700 dark:border-[#3a3a44] dark:text-slate-300">Phase 1 (fetchJoin)</td><td class="border-t border-slate-100 px-3 py-2 font-semibold text-blue-600 dark:border-[#3a3a44] dark:text-blue-400">2번</td><td class="border-t border-slate-100 px-3 py-2 dark:border-[#3a3a44]"><img src="phase1-scouter-sql2.png" alt="Phase1 SQL 2" class="h-auto max-h-24 rounded object-contain"/></td></tr><tr><td class="border-t border-slate-100 px-3 py-2 text-slate-700 dark:border-[#3a3a44] dark:text-slate-300">Phase 2-A (Cache Hit)</td><td class="border-t border-slate-100 px-3 py-2 font-semibold text-emerald-600 dark:border-[#3a3a44] dark:text-emerald-400">0번</td><td class="border-t border-slate-100 px-3 py-2 dark:border-[#3a3a44]"><img src="phase2a-scouter-cache-hit1.png" alt="Phase2A Cache Hit" class="h-auto max-h-24 rounded object-contain"/></td></tr></tbody></table></div></div>',
        result:
          'QueryDSL fetchJoin으로 1차 쿼리 최적화 후, @Cacheable + Redis 캐싱을 추가 적용해 최종 개선<div class="mt-3 overflow-hidden rounded-xl border border-slate-200/95 bg-white shadow-[0_6px_28px_-6px_rgba(30,58,95,0.14)] dark:border-[#42424c] dark:bg-[#2c2c34]"><table class="w-full table-fixed border-collapse text-left text-base text-slate-700 dark:text-slate-300"><caption class="sr-only">성능 개선 결과</caption><thead><tr class="border-b border-slate-200 bg-slate-50 dark:border-[#40404a] dark:bg-[#32323a]"><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">항목</th><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">Before</th><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">After</th><th class="px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300">개선율</th></tr></thead><tbody><tr><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-slate-900 dark:border-[#3a3a44] dark:text-white">응답시간 (V100)</td><td class="border-t border-slate-100 px-3 py-2.5 tabular-nums text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">645ms</td><td class="border-t border-slate-100 px-3 py-2.5 font-semibold tabular-nums text-blue-700 dark:border-[#3a3a44] dark:text-blue-300">209ms</td><td class="border-t border-slate-100 px-3 py-2.5 font-bold text-emerald-600 dark:border-[#3a3a44] dark:text-emerald-400">-68%</td></tr><tr><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-slate-900 dark:border-[#3a3a44] dark:text-white">응답시간 (V1000)</td><td class="border-t border-slate-100 px-3 py-2.5 tabular-nums text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">6,818ms</td><td class="border-t border-slate-100 px-3 py-2.5 font-semibold tabular-nums text-blue-700 dark:border-[#3a3a44] dark:text-blue-300">638ms</td><td class="border-t border-slate-100 px-3 py-2.5 font-bold text-emerald-600 dark:border-[#3a3a44] dark:text-emerald-400">-91%</td></tr><tr><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-slate-900 dark:border-[#3a3a44] dark:text-white">TPS (V100)</td><td class="border-t border-slate-100 px-3 py-2.5 tabular-nums text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">154.6</td><td class="border-t border-slate-100 px-3 py-2.5 font-semibold tabular-nums text-blue-700 dark:border-[#3a3a44] dark:text-blue-300">470.1</td><td class="border-t border-slate-100 px-3 py-2.5 font-bold text-emerald-600 dark:border-[#3a3a44] dark:text-emerald-400">+204%</td></tr><tr><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-slate-900 dark:border-[#3a3a44] dark:text-white">TPS (V1000)</td><td class="border-t border-slate-100 px-3 py-2.5 tabular-nums text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">138.7</td><td class="border-t border-slate-100 px-3 py-2.5 font-semibold tabular-nums text-blue-700 dark:border-[#3a3a44] dark:text-blue-300">438.3</td><td class="border-t border-slate-100 px-3 py-2.5 font-bold text-emerald-600 dark:border-[#3a3a44] dark:text-emerald-400">+216%</td></tr><tr><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-slate-900 dark:border-[#3a3a44] dark:text-white">분산 환경 캐시</td><td class="border-t border-slate-100 px-3 py-2.5 text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">서버 재시작 시 소실</td><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-blue-700 dark:border-[#3a3a44] dark:text-blue-300">Redis 유지</td><td class="border-t border-slate-100 px-3 py-2.5 text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">-</td></tr><tr><td class="border-t border-slate-100 px-3 py-2.5 font-semibold text-slate-900 dark:border-[#3a3a44] dark:text-white">SQL Count</td><td class="border-t border-slate-100 px-3 py-2.5 tabular-nums text-slate-600 dark:border-[#3a3a44] dark:text-slate-400">42번</td><td class="border-t border-slate-100 px-3 py-2.5 font-semibold tabular-nums text-blue-700 dark:border-[#3a3a44] dark:text-blue-300">0번 (Cache Hit)</td><td class="border-t border-slate-100 px-3 py-2.5 font-bold text-emerald-600 dark:border-[#3a3a44] dark:text-emerald-400">-100%</td></tr></tbody></table></div><div class="mt-4 grid grid-cols-2 gap-3"><div><p class="mb-1.5 text-sm font-semibold text-slate-500 dark:text-slate-400">Before — V100</p><img src="before-ngrinder-v100.png" alt="Before nGrinder V100" class="w-full rounded-lg border border-slate-200 object-contain dark:border-[#4a4a52]"/></div><div><p class="mb-1.5 text-sm font-semibold text-slate-500 dark:text-slate-400">After — V100</p><img src="phase2a-ngrinder-v100.png" alt="After nGrinder V100" class="w-full rounded-lg border border-slate-200 object-contain dark:border-[#4a4a52]"/></div><div><p class="mb-1.5 text-sm font-semibold text-slate-500 dark:text-slate-400">Before — V1000</p><img src="before-ngrinder-v1000.png" alt="Before nGrinder V1000" class="w-full rounded-lg border border-slate-200 object-contain dark:border-[#4a4a52]"/></div><div><p class="mb-1.5 text-sm font-semibold text-slate-500 dark:text-slate-400">After — V1000</p><img src="phase2a-ngrinder-v1000.png" alt="After nGrinder V1000" class="w-full rounded-lg border border-slate-200 object-contain dark:border-[#4a4a52]"/></div></div>',
      },
    ],
