...
export const PROJECTS: ProjectData[] = [
  {
    ...
    serviceOverview:
      '\'내가 사용할 서비스인가\'를 기준으로 설계한, 구매한 신발을 피드로 공유하고 이벤트·투표·리워드로 재방문을 유도하는 선순환 구조의 커뮤니티 기반 패션 커머스 플랫폼\n구매 인증 기반으로 이벤트에 참여하고, 피드 투표로 순위를 겨루며, 상위 랭커에게 포인트·쿠폰 보상이 지급되는 구조로 단순 구매에서 커뮤니티 재참여까지 이어지도록 설계했습니다.\n사용자가 이벤트를 탐색하는 단계에서 이탈하면 선순환 구조 자체가 끊기기 때문에, 이벤트·피드 도메인의 성능과 동시성 문제 해결이 핵심 과제였습니다.',
