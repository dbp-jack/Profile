...
`/manage`는 로컬에서만 쓰는 **포트폴리오 작업실**입니다. 프로젝트·구성 → 소개 문구 → 지원 기업 → 검토·공유의 네 단계로 편집합니다.

- 편집과 미리보기를 넓게 나란히 표시하고, 좁은 화면에서는 두 화면을 전환합니다. 미리보기는 읽기 크기·PC·모바일과 큰 창을 지원합니다.
- 프로젝트 두 칸에서 바로 교체·순서 변경하고, 기업 이름만 입력해 기업별 문구를 작성할 수 있습니다.
- 선택과 문구를 자동 저장하고, 이름 있는 구성 저장·불러오기, 직전 변경 취소, 기본값 복원을 제공합니다. 기존 저장 자료는 유지합니다.
- 저장 계약과 검증 범위는 [작업실 README](src/pages/manage/README.md)를 참고합니다.

- `src/pages/manage/`는 저장소에 포함해 새 작업도 canonical workspace에서 같은 최신 관리 도구를 사용합니다.
...
- 이전 로컬 `/web-preview`는 `/`로, `/pdf-compare`는 `/pdf`로 이동합니다. 쿼리·페이지 번호·위치 링크를 보존하며 별도 시안 소스는 유지하지 않습니다.
